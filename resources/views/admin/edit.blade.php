...
<head>
    ...
    <title>Edit Blog | BizCashAndCapital</title>
        ...
        <section style="padding: 2rem 2rem 1rem 2rem; background: linear-gradient(135deg, #1a7a4a, #0f5a38); position: relative;">
            ...
            <div style="max-width: 1280px; margin: 0 auto; position: relative; z-index: 2;">
                <div style="display: inline-flex; align-items: center; gap: 0.5rem; background: rgba(255,255,255,0.15); padding: 0.35rem 1rem; margin-bottom: 1rem; border-radius: 40px !important;">
                    ...
                    <span style="font-size: 0.65rem; font-weight: 700; color: white; letter-spacing: 0.08em;">EDIT BLOG</span>
                </div>
                <h1 style="font-size: clamp(1.8rem, 5vw, 2.5rem); font-weight: 800; color: white; font-family: 'Space Grotesk', sans-serif;">Edit Blog Post</h1>
                <div style="width: 60px; height: 3px; background: linear-gradient(90deg, #c8e86a, white); margin: 0.75rem 0;"></div>
                <p style="color: rgba(255,255,255,0.85); font-size: 0.875rem;">Update your blog post details below</p>
            </div>
        </section>
        ...
        <main style="padding: 2rem;">
            <div style="max-width: 900px; margin: 0 auto;">
                <div style="background: white; border: 1px solid #e2e8e2; padding: 2rem;">
                    <form id="editBlogForm" action="{{ route('update-blogs', $blog->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div style="margin-bottom: 1.5rem;">
                            ...
                            <input type="text" id="title" name="title" class="form-input" placeholder="Enter blog title" value="{{ old('title', $blog->title) }}" required>
                            @error('title')
                        ...
                        <div style="margin-bottom: 1.5rem;">
                            ...
                            <input type="text" id="slug" name="slug" class="form-input" placeholder="enter-blog-title-here" value="{{ old('slug', $blog->slug) }}" required>
                        ...
                        <div style="margin-bottom: 1.5rem;">
                            ...
                            <select id="category" name="category" class="form-input" required>
                                ...
                                <option value="Finance" {{ old('category', $blog->category) == 'Finance' ? 'selected' : '' }}>Finance</option>
                                <option value="Loans" {{ old('category', $blog->category) == 'Loans' ? 'selected' : '' }}>Loans</option>
                                <option value="SBA" {{ old('category', $blog->category) == 'SBA' ? 'selected' : '' }}>SBA</option>
                                <option value="Tips" {{ old('category', $blog->category) == 'Tips' ? 'selected' : '' }}>Tips & Advice</option>
                            </select>
                            @error('category')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div style="margin-bottom: 1.5rem;">
                            ...
                            <div id="currentImagePreview" style="margin-top: 0.5rem; position: relative; display: inline-block;">
                                <img id="currentPreviewImg" src="{{ asset('storage/blogs/' . $blog->image) }}" alt="Current image" style="max-width: 100%; max-height: 250px; object-fit: cover; border: 1px solid #e2e8e2;">
                                <button type="button" id="removeCurrentImageBtn" class="remove-image-btn" style="background: #ef4444;">×</button>
                            </div>
                            <input type="hidden" id="remove_image" name="remove_image" value="0">
                            <p style="font-size: 0.7rem; color: #9ca3af; margin-top: 0.5rem;">Click × to remove current image and upload a new one</p>
                        </div>
                        @endif
                        ...
                        <div style="margin-bottom: 1.5rem;">
                            ...
                            <div class="file-upload-area" id="fileUploadArea">
                                ...
                                <p style="color: #64748b; font-size: 0.875rem;">Click or drag & drop to upload new image</p>
                        ...
                        <div style="margin-bottom: 1.5rem;">
                            ...
                            <textarea id="meta_description" name="meta_description" rows="3" class="form-input" placeholder="Write a brief description for search engines (150-160 characters recommended)" maxlength="160">{{ old('meta_description', $blog->meta_description) }}</textarea>
                        ...
                        <div style="margin-bottom: 1.5rem;">
                            ...
                            <textarea id="content" name="content" rows="12" class="form-input" placeholder="Write your blog content here..." required>{{ old('content', $blog->content) }}</textarea>
                            @error('content')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div style="display: flex; gap: 1rem; justify-content: flex-end; border-top: 1px solid #e2e8e2; padding-top: 1.5rem;">
                            <a href="{{ route('admin.blogs.index') }}" class="btn-secondary" style="text-decoration: none;">Cancel</a>
...
<script>
    ...
    if (removeCurrentImageBtn) {
        removeCurrentImageBtn.addEventListener('click', function() {
            if (confirm('Remove current image? You can upload a new one.')) {
